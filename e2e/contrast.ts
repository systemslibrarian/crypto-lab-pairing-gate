 ...
 *
 *  - text over a surface axe declines to resolve — a gradient, or a
 *    `color-mix()`. This lab paints the hero's "why it matters" card, every
 *    verdict banner and every badge tint from `color-mix()` backgrounds, so
 *    axe files those nodes under `incomplete` rather than measuring them.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The hero subtitle rendered
 *    at `opacity:.85` on top of an inherited `--text-muted`, and disabled
 *    buttons drop to `.5`. Note this lab collapses its animations to 0.01ms
 *    under reduced motion, which preserves end states, and gates the signer-card
 *    merge on `no-preference` so it never runs at all under the gate.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The revealed G_T
 *    values are 576 bytes of hex inside `overflow-x: auto` blocks, and the
 *    signer grid scrolls sideways too, so most of that text sits outside its
 *    scroller's client box at any moment. Content scrolled past that box is not
 *    dimmed or partly drawn — it is absent from the frame, and asking what
 *    colour it sits on has no answer. Its rect is still outside every
 *    ancestor's box, so the ancestor walk would find nothing behind it and fall
 *    through to white, inventing a failure. Skip it, and rely on the hex that
 *    IS in view, which is measured for real.
 *
 ...
 *    draws its aggregation diagram out of divs, and its one figure — the
 *    hash-to-curve sketch in `.htc-curve` — is a `<path>` and a `<circle>` with
 *    no `<text>` at all, so this rule finds nothing to composite here today. It
 *    is kept because it is cheap and silent when unused: an `<svg>` draws a
 *    shape as a `<rect>` and then its label as a FOLLOWING `<text>`, so a
 *    label's backdrop is a preceding sibling, not an ancestor. An
 *    ancestor-only walk would composite such labels onto the svg's own
 *    transparent background and report a ratio nothing on screen has.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: the hero's "why it
     * matters" wash is authored with `color-mix()`, and Chromium reports that
     * to `getComputedStyle` unchanged rather than converting it to sRGB. A
     ...
     * fabricates failures (a dark-on-light packet read as dark-on-dark) and, far
     * worse, could hide a real one. The 2D canvas is the browser's own colour
     ...
     * gradient whose worst stop spans its element. This lab's `.load-progress-
     * fill` is a four-stop `linear-gradient(90deg, transparent, --input-focus,
     * --tag-text, transparent)` sliding inside a 0.42rem track: judged at its
     * worst stop it reports `transparent`, i.e. the track behind it, for a strip
     * that is fully saturated across its middle. So each gradient is *sampled*
     * at the text's real location instead: linear by projecting onto the
     * gradient line, radial by distance from the centre over the
     * farthest-corner radius. A non-gradient layer (`url()`, `none`) is
     * unmeasurable and paints nothing. The radial branch finds nothing to do on
     * this page today; it is kept because it is silent when unused and the
     * alternative is a helper that silently mismeasures the first radial wash
     * anyone adds.
     */
     ...
     *
     * A driven scan here walks four sections at once, and the revealed state
     * puts 576 bytes of G_T hex on BOTH sides of the pairing equation plus a
     * grid of signer cards, so every one of those nodes re-walks the same
     * handful of ancestors up to `<body>`. Without the caches the pass
     * re-reads the same computed styles and rects thousands of times, and the
     * gate scans a couple of dozen states per configuration. Nothing mutates
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. Step 1 of this lab's schematic draws its permutation
      // arrows as `<path fill="none">`, which `resolve` returns null for, so
      // they are skipped for the same reason by a different route.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has exactly one <details>, the field-tower
      // explainer, and the gate OPENS it and scans the revealed state rather
      // than measuring it shut. `checkVisibility()` covers the rest: the G_T
      // reveal blocks and every section's output region are built and torn down
      // by their own handlers, so the gate reaches each state by driving it
      // instead of unhiding anything.
      ...
      // header's `.cl-skip-link` parks above the viewport and slides in only on
     ...
     * it, the `aria-hidden` decoration this lab carries — the button spinners,
     * the load-progress bar, the histogram callout's ▲ arrow and the
     * `#mechanism-marker` overlay — would be measured as if each took its
     ...
     *
     * This lab's one SVG figure is written as a literal in a template with no
     * interpolation inside it, so there are no stray text nodes today. The
     * guard stays because the mistake
     * is cheap to make and silent: interpolating a string ARRAY into a template
     * literal (`<g>${dots}</g>`) makes JS join it with commas, leaving a run of
     * "," text nodes inside the group. They are invisible on screen, but a walk
     * that measures them reports a 1:1 failure against the panel — a ratio
